...
const SHIFT_TYPE_CONFIG: Record<
  ShiftType,
  { label: string; icon: React.ComponentType<{ size?: number; color?: string; className?: string }>; color: string; bg: string; border: string; desc: string }
> = {
  day: {
    label: 'Дневная',
    icon: IconDayShift,
    ...
    <div className="w-full h-full bg-[#0d0e14] flex flex-col overflow-hidden select-none text-[#e2e8f0]">
      ...
      <div className="p-3 border-b border-white/[0.08] bg-[#111217]/95 backdrop-blur-md flex flex-col gap-2.5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-2xl bg-[#8b5cf6]/20 text-[#8b5cf6] border border-[#8b5cf6]/30 flex items-center justify-center shadow-lg shadow-purple-500/10">
              <Calendar size={18} />
            </div>
            <div>
              ...
              <p className="text-[11px] text-[#94a3b8]">
                Календарь смен, дел, стрижек и заработка
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-2 rounded-xl bg-[#161720] border border-white/[0.08] text-[#94a3b8] hover:text-white transition-colors"
            ...
            <div className="flex items-center gap-1 bg-[#161720] border border-white/[0.08] p-1 rounded-xl shadow-sm">
              ...
              <select
                ...
                className="bg-transparent text-xs font-bold text-[#8b5cf6] focus:outline-none cursor-pointer px-1 py-0.5 font-mono"
              >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-[#8b5cf6]/20 hover:bg-[#8b5cf6]/30 text-[#8b5cf6] border border-[#8b5cf6]/35 text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm"
            >
              <Sparkles size={13} />
              <span>Генератор графика</span>
            </button>

            {/* Granular Cleanup Modal Button */}
            <button
              onClick={() => setIsCleanupModalOpen(true)}
              className="px-3 py-1.5 rounded-xl bg-red-500/15 hover:bg-red-500/25 text-[#f43f5e] border border-red-500/30 text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm"
              title="Выборочное и массовое удаление смен и дел"
            >
              <Eraser size={13} />
              <span>Очистить смены/дела</span>
            </button>

            {/* Beautiful Report Button */}
            <button
              onClick={() => setIsReportModalOpen(true)}
              className="px-3 py-1.5 rounded-xl bg-[#10b981]/20 hover:bg-[#10b981]/30 text-[#10b981] border border-[#10b981]/35 text-xs font-semibold flex items-center gap-1.5 transition-all shadow-sm"
            >
              <PieChart size={13} />
              <span>Красивый отчет</span>
            </button>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-3 overflow-y-auto">
          ...
          <div className="flex items-center gap-1 p-1.5 mb-2 bg-[#14151c] border border-white/[0.08] rounded-xl text-xs overflow-x-auto shadow-sm shrink-0">
            <span className="text-[10px] text-[#94a3b8] font-bold px-1.5 uppercase tracking-wider shrink-0">
               Кисть:
            </span>
            <button
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1 transition-all border ml-auto shrink-0 ${
            ...
            <div className="mb-2 p-2 px-3 bg-red-500/15 border border-red-500/40 rounded-xl text-xs flex items-center justify-between gap-2 animate-fade-in flex-wrap shadow-md shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white font-semibold">
                  Ластик: <span className="text-[#cbd5e1] font-normal">зажмите мышь и ведите по дням, чтобы быстро стереть</span>
                </span>
              </div>
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-[#f43f5e] border border-red-500/40 text-xs font-bold transition-all"
                >
                  Стереть все смены месяца
                </button>
                ...
                <button
                  onClick={() => {
                    ...
                    const count = shifts.filter((s) => s.date.startsWith(monthPrefix) && (s.type === 'off' || s.type === 'vacation')).length;
                    shifts.filter((s) => s.date.startsWith(monthPrefix) && (s.type === 'off' || s.type === 'vacation')).forEach((s) => deleteShift(s.date));
                    showToast(`Стерты все выходные (${count} шт.)`);
                  }}
                  className="px-2.5 py-1 rounded-lg bg-white/[0.06] hover:bg-white/[0.12] text-[#cbd5e1] border border-white/[0.08] text-xs font-semibold transition-all"
                >
                  Стереть выходные
                </button>
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-red-600 hover:bg-red-700 text-white text-xs font-bold transition-all shadow-md"
                >
                  Очистить весь месяц
                </button>
              </div>
          ...
          {/* Weekdays Header */}
          <div className="grid grid-cols-7 gap-2 mb-1.5 shrink-0">
            {WEEKDAY_NAMES.map(({ short }, idx) => (
              <div
                key={short}
                className={`text-center text-xs font-bold py-1 rounded-xl ${
            ...
            {calendarDays.map(({ dateStr, dayNum, isCurrentMonth, dayOfWeek }) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[64px] sm:min-h-[74px] 2xl:min-h-[86px] p-2 sm:p-2.5 rounded-2xl border flex flex-col justify-between transition-all cursor-pointer relative group select-none ${
                    isSelected
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span
                      className={`text-xs font-bold w-5.5 h-5.5 sm:w-6 sm:h-6 rounded-lg flex items-center justify-center ${
                        isToday
                    ...
                    {config && shift && (
                      <span
                        className="px-1.5 sm:px-2 py-0.5 rounded-lg text-[9px] sm:text-[10px] font-bold border flex items-center gap-1 shadow-sm truncate max-w-[85px] sm:max-w-none"
                        style={{
                  ...
                  {/* Middle Section: Shift Hours & Event Chips */}
                    ...
                    {dayEvts.length > 0 && (
                      <div className="space-y-0.5">
                        {dayEvts.slice(0, 2).map((ev) => (
                          <div
                            key={ev.id}
                            className="px-1.5 py-0.5 rounded-md bg-white/[0.06] text-[9px] sm:text-[10px] text-white truncate flex items-center gap-1 border border-white/[0.08]"
                          >
                            <span
                  ...
                  {/* Bottom Bar: Clean Financial Earnings Pill (No redundant duplicate labels) */}
                  <div className="flex items-center justify-between pt-0.5">
                    {shift && shift.type !== 'off' && shift.type !== 'vacation' ? (
                      <div className="flex items-center gap-1">
                        <span className="px-1.5 py-0.5 rounded-md bg-[#10b981]/15 border border-[#10b981]/30 text-[10px] sm:text-[11px] font-extrabold text-[#10b981] font-mono shadow-sm">
                          +{shift.earnings.toLocaleString('ru-RU')} ₽
                        </span>
                      </div>
                    ) : (
                      <span />
        ...
        <div className="w-80 2xl:w-84 shrink-0 border-l border-white/[0.08] bg-[#111217] p-4 flex flex-col justify-between overflow-y-auto">
            ...
            {inspectorTab === 'shift' ? (
              <div className="space-y-3">
                <div className="space-y-1">
                  <label className="text-[11px] font-semibold text-[#94a3b8]">Тип смены</label>
                        ...
                        <button
                          ...
                          className={`p-2 rounded-xl border text-xs font-bold flex flex-col items-center gap-1 transition-all ${
                            isCur
                              ...
                              : 'border-white/[0.06] bg-[#161720] text-[#94a3b8] hover:bg-white/[0.06]'
                          }`}
                        >
                ...
                {formType !== 'off' && formType !== 'vacation' && (
                  <div className="space-y-2.5 pt-2 border-t border-white/[0.06]">
                    <div>
                      <label className="text-xs font-medium text-[#94a3b8] flex items-center justify-between mb-1">
                        <span>Отработано часов:</span>
                        <strong className="text-white font-mono text-sm">{formHours} ч</strong>
                      </label>
                      <input
                        type="range"
                        min="1"
                    ...
                    {/* Simple Unified Shift Expense Box with clean wrapping adaptation */}
                    <div className="p-3 rounded-2xl bg-[#161720] border border-white/[0.08] space-y-2.5">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                        {[100, 200, 300, 500, 1000].map((amt) => (
                          <button
                            ...
                            className="flex-1 min-w-[42px] py-1 px-1.5 rounded-lg bg-white/[0.06] hover:bg-white/[0.14] text-[11px] font-mono text-[#cbd5e1] font-bold border border-white/[0.06] hover:border-white/20 transition-all text-center"
                          >
                            +{amt}
                          </button>
                        ))}
                        {formExpense > 0 && (
                          <button
                            ...
                            className="py-1 px-2 rounded-lg bg-red-500/15 hover:bg-red-500/25 text-[#f43f5e] text-[10px] font-bold border border-red-500/20 transition-all"
                      ...
                      <label className="flex items-center gap-2 pt-0.5 cursor-pointer">
                        ...
                        <span className="text-[10px] text-[#94a3b8]">
                          Вносить трату в модуль Финансов (Расход)
                        </span>
                      </label>
                    </div>

                    {/* Net Earnings Summary Box */}
                    <div className="p-3 rounded-2xl bg-[#161720] border border-white/[0.08] space-y-1 shadow-md">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-[#94a3b8]">Начислено:</span>
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-[#94a3b8] mb-0.5">
                    Примечание к смене
                  </label>
                  <textarea
                    value={formNote}
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-[#8b5cf6] text-white text-xs font-bold flex items-center justify-center gap-2 shadow-lg shadow-purple-500/25 hover:bg-[#8b5cf6]/90 transition-all"
                >
                  <Save size={14} />
                  <span>Сохранить смену и трату</span>
                </button>
              </div>
            ) : (
