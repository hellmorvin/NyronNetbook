  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 left-3 z-20 pointer-events-none">
        <div
          className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl text-xs"
          style={{
            ...
            boxShadow: isLight ? '0 2px 16px rgba(0,0,0,0.08)' : '0 2px 24px rgba(0,0,0,0.4)',
          }}
        >
          <IconGraph2D size={14} color={currentAccent} />
          <span style={{ color: isLight ? '#0f172a' : '#f1f5f9', fontWeight: 700 }}>Граф знаний</span>
          <span style={{ width: 1, height: 12, background: isLight ? 'rgba(0,0,0,0.12)' : 'rgba(255,255,255,0.12)', display: 'inline-block' }} />
          <span style={{ color: isLight ? '#334155' : '#94a3b8' }}>
            <span style={{ color: isLight ? '#0f172a' : '#e2e8f0', fontWeight: 700, fontFamily: 'monospace' }}>{neurons.length}</span> мыслей
          </span>
          <span style={{ width: 4, height: 4, borderRadius: '50%', background: isLight ? 'rgba(0,0,0,0.15)' : 'rgba(255,255,255,0.15)', display: 'inline-block' }} />
          <span style={{ color: isLight ? '#334155' : '#94a3b8' }}>
            <span style={{ color: currentAccent, fontWeight: 700, fontFamily: 'monospace' }}>{totalSynapses}</span> связей
          </span>
        </div>
      </div>

      {/* ═══ Search Bar (top-center, expands on focus) ════════════════ */}
        ...
        >
          ...
          <input
            ...
            style={{
              ...
              borderRadius: 12,
              paddingLeft: 30,
              paddingRight: graphSearchQuery ? 28 : 12,
              paddingTop: 6,
              paddingBottom: 6,
              fontSize: 12,
              width: graphSearchQuery ? 220 : 180,
      ...
      {allTags.length > 0 && (
        <div
          className="absolute bottom-3 left-3 z-20 flex items-center gap-1.5 p-1.5 rounded-xl max-w-[70%] overflow-x-auto no-scrollbar"
          style={{
            background: isLight ? 'rgba(255,255,255,0.88)' : 'rgba(18,18,24,0.88)',
        ...
        >
          <div className="flex items-center gap-1 px-1 text-[10px] font-bold tracking-wider" style={{ color: isLight ? '#94a3b8' : '#64748b' }}>
            <Filter size={10} />
            <span>Теги</span>
          </div>
          ...
          {allTags.map((tag) => {
            ...
            return (
              <button
                ...
                className="px-2.5 py-0.5 rounded-lg text-[11px] transition-all flex items-center gap-1 shrink-0"
                style={{
                  background: isSelected ? currentAccent : (isLight ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.06)'),
                  color: isSelected ? '#fff' : (isLight ? '#475569' : '#94a3b8'),
                  border: isSelected
              ...
              >
                #{tag}
                {isSelected && <X size={10} className="ml-0.5 opacity-75" />}
              </button>
            );
          })}
        </div>
