...
const defaultFolders: FolderItem[] = [
  { id: 'fld_neuro', name: 'Нейробиология', isOpen: true },
  { id: 'fld_ai', name: 'Нейросети_и_AI', isOpen: true },
  { id: 'fld_work', name: 'Работа_и_Финансы', isOpen: true },
  { id: 'fld_ideas', name: 'Идеи_и_Проекты', isOpen: false },
];

const initialTabs: TabItem[] = [
  { id: 'tab_graph', type: 'graph', title: 'Граф' },
  { id: 'tab_note_1', type: 'note', noteId: initialWithLinks[0]?.id, title: initialWithLinks[0]?.title || 'Без названия' },
];
...
const initialCanvasCards: CanvasCard[] = [
  {
    ...
    height: 190,
    type: 'sticky',
    title: 'Архитектура Второго Мозга',
    content: '1. Быстрый сбор мыслей\n2. Нейро-связи [[Концепция]]\n3. Анализ и интервальное повторение\n4. Автономный учет работы и финансов',
    color: '#7c5cff',
  },
  {
    id: 'card_2',
    ...
    height: 190,
    type: 'note',
    title: 'Долговременная потенциация (LTP)',
    content: 'Стойкое усиление синаптической передачи между нейронами. Клеточная основа памяти и обучения.',
    noteId: 'neu_ltp_01',
    color: '#f59e0b',
  },
  {
    ...
    type: 'note',
    title: 'Правило Хебба',
    content: 'Нейроны, которые возбуждаются вместе, связываются вместе.',
    noteId: 'neu_hebb_02',
    color: '#38bdf8',
  },
  {
    ...
    height: 180,
    type: 'sticky',
    title: 'График Смен и Финансы',
    content: '• Смена: 12ч × 450 ₽ = 5 400 ₽\n• График: 2/2 сменный\n• Финансовый учет расходов и накоплений',
    color: '#10b981',
  },
];
...
const generateInitialShifts = (): WorkShift[] => {
  ...
  const shiftPlan = [
    { day: 1, type: 'day' as ShiftType, hours: 12, rate: 450, bonus: 0 },
    { day: 2, type: 'night' as ShiftType, hours: 12, rate: 500, bonus: 0 },
    { day: 3, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 4, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 5, type: 'day' as ShiftType, hours: 12, rate: 450, bonus: 0 },
    { day: 6, type: 'night' as ShiftType, hours: 12, rate: 500, bonus: 0 },
    { day: 7, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 8, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 9, type: 'day' as ShiftType, hours: 12, rate: 450, bonus: 0 },
    { day: 10, type: 'night' as ShiftType, hours: 12, rate: 500, bonus: 0 },
    { day: 11, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 12, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 13, type: 'day' as ShiftType, hours: 12, rate: 450, bonus: 0 },
    { day: 14, type: 'night' as ShiftType, hours: 12, rate: 500, bonus: 0 },
    { day: 15, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 16, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 17, type: 'day' as ShiftType, hours: 12, rate: 450, bonus: 0 },
    { day: 18, type: 'night' as ShiftType, hours: 12, rate: 500, bonus: 0 },
    { day: 19, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 20, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 21, type: 'day' as ShiftType, hours: 12, rate: 450, bonus: 0 },
    { day: 22, type: 'night' as ShiftType, hours: 12, rate: 500, bonus: 0 },
    { day: 23, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 24, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 25, type: 'day' as ShiftType, hours: 12, rate: 450, bonus: 0 },
    { day: 26, type: 'night' as ShiftType, hours: 12, rate: 500, bonus: 0 },
    { day: 27, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
    { day: 28, type: 'off' as ShiftType, hours: 0, rate: 0, bonus: 0 },
  ];

  shiftPlan.forEach((p) => {
    const dateStr = `${year}-${pad(month + 1)}-${pad(p.day)}`;
    const earnings = p.hours > 0 ? p.hours * p.rate + p.bonus : 0;
    shifts.push({
      id: `shift_${dateStr}`,
      date: dateStr,
      type: p.type,
      ...
      bonus: p.bonus,
      earnings,
      note: p.type === 'day' ? 'Дневная смена 12ч' : p.type === 'night' ? 'Ночная смена 12ч' : 'Выходной',
    });
  });
...
const generateInitialEvents = (): CalendarEvent[] => {
  ...
  return [
    {
      id: 'evt_1',
      date: `${year}-${pad(month + 1)}-06`,
      time: '15:00',
      title: 'Стрижка и уход',
      category: 'beauty',
      amount: 1500,
      completed: true,
      color: '#ec4899',
...
const generateInitialTransactions = (): FinanceTransaction[] => {
  ...
  return [
    ...
    {
      id: 'tx_2',
      date: `${year}-${pad(month + 1)}-07`,
      type: 'expense',
      category: 'Еда/Продукты',
      amount: 6800,
      description: 'Продукты на неделю + доставка',
      paymentMethod: 'Карта',
    },
    {
      id: 'tx_3',
      ...
      amount: 54000,
      description: 'Аванс за первую половину месяца (10 смен)',
    ...
    },
    {
      ...
      category: 'Жилье/ЖКХ',
      amount: 28000,
      description: 'Аренда жилья за месяц',
      paymentMethod: 'СБП',
    },
    {
      id: 'tx_5',
    ...
    },
    {
      ...
      category: 'Подработка',
      amount: 12500,
      description: 'Фриланс / дополнительная смена',
      paymentMethod: 'СБП',
    },
  ];
};
...
const initialSavingsGoals: SavingsGoal[] = [
  {
    id: 'goal_1',
    title: 'Новый ноутбук / ПК для работы',
    targetAmount: 140000,
    currentAmount: 85000,
    deadline: '2026-11-30',
    category: 'Техника',
    color: '#7c5cff',
    note: 'Мощный сетап для работы и нейросетей',
  },
  {
    id: 'goal_2',
    title: 'Подушка финансовой безопасности (3 мес)',
    targetAmount: 180000,
    currentAmount: 120000,
    deadline: '2026-12-31',
    category: 'Резерв',
    color: '#10b981',
    note: 'Неприкосновенный запас на непредвиденные случаи',
  },
  {
    id: 'goal_3',
    title: 'Отпуск и путешествие',
    targetAmount: 90000,
    currentAmount: 35000,
    deadline: '2026-10-15',
    category: 'Отдых',
    color: '#38bdf8',
    note: 'Билеты, отель и карманные расходы',
  },
];
...
const initialBankDeposits: BankDeposit[] = [
  {
    ...
    notes: 'Ежемесячная выплата процентов с капитализацией',
    color: '#10b981',
  },
  {
    id: 'dep_2',
    title: 'Срочный вклад «Максимальный доход»',
    bankName: 'Сбербанк',
    balance: 150000,
    interestRate: 19.5,
    ...
    notes: 'Фиксированная ставка на 6 месяцев',
    color: '#f59e0b',
  },
];
