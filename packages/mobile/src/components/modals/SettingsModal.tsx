...
export const SettingsModal: React.FC = () => {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-3xl h-full max-h-[94vh] sm:max-h-[85vh] bg-[#14151c] rounded-3xl border border-white/[0.12] shadow-2xl overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-4 py-3 sm:px-6 sm:h-14 border-b border-white/[0.08] flex items-center justify-between bg-[#171822] shrink-0">
          <div className="flex items-center gap-2.5">
            <NeuralNotebookLogo size={22} glow={false} />
            <div>
              <h2 className="text-xs sm:text-sm font-extrabold text-white tracking-wide">Параметры NyronNotebook</h2>
              <p className="text-[10px] sm:text-[11px] text-[#94a3b8]">Настройка тем, внешнего вида и P2P синхронизации</p>
            </div>
          </div>
          <button
            onClick={() => setSettingsOpen(false)}
            className="w-8 h-8 rounded-full bg-white/[0.06] hover:bg-white/[0.12] flex items-center justify-center text-[#94a3b8] hover:text-white transition-colors"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-3 py-2 border-b border-white/[0.08] bg-[#0f1015] flex items-center gap-1.5 overflow-x-auto no-scrollbar shrink-0 text-xs">
          <button
            onClick={() => setActiveTab('themes')}
            className={`px-3 py-2 rounded-xl font-bold transition-all flex items-center gap-1.5 shrink-0 text-xs ${
              activeTab === 'themes'
                ? 'bg-[#7c5cff] text-white shadow-md'
                : 'text-[#94a3b8] hover:text-white bg-white/[0.03]'
            }`}
          >
            <Palette size={14} />
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl font-bold text-[#38bdf8] bg-[#38bdf8]/10 hover:bg-[#38bdf8]/20 border border-[#38bdf8]/20 transition-all flex items-center gap-1.5 shrink-0 text-xs"
          >
        ...
        {/* Content Area */}
        <div className="flex-1 p-3.5 sm:p-6 overflow-y-auto text-xs space-y-4 bg-[#14151c]">
          {/* ================= TAB: THEMES ================= */}
          {activeTab === 'themes' && (
            <div className="space-y-4 animate-fade-in">
              {/* 1. Theme Mode 3-Way Switcher */}
              <div className="p-3.5 sm:p-4 rounded-2xl bg-[#171822] border border-white/[0.08] space-y-2.5">
                <div>
                  <h4 className="font-bold text-white text-xs mb-0.5">Режим темы интерфейса</h4>
                  <p className="text-[11px] text-[#94a3b8]">
                    Принудительный темный, светлый или системный режим.
                  </p>
                </div>
