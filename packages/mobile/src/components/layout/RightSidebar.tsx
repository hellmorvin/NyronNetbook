      ...
      <div className="h-10 px-3 border-b border-white/[0.08] flex items-center justify-between">
        <div className="flex gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-semibold py-1 px-2 rounded-md transition-all ${
          ...
          >
            <Settings2 size={13} className="text-[#38bdf8]" />
            <span>Граф</span>
          </button>
        </div>
        <button
          onClick={toggleRightSidebar}
          className="p-1 hover:bg-white/[0.08] rounded text-[#94a3b8] hover:text-white transition-colors"
          title="Скрыть панель"
        >
              ...
              <div className="space-y-3">
                <div>
                  <span className="text-[11px] text-[#64748b] block mb-1">
                    Исходящие связи ({(activeNeuron.wikiLinks || []).length}):
                  </span>
                  <div className="space-y-1">
                    {(activeNeuron.wikiLinks || []).map((targetTitle) => {
                      ...
                      return (
                        <div
                          key={targetTitle}
                          onClick={() =>
                            target && openTab({ type: 'note', noteId: target.id, title: target.title })
                          }
                    ...
                    {(!activeNeuron.wikiLinks || activeNeuron.wikiLinks.length === 0) && (
