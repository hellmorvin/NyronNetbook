...
<p align="center">
  <img src="https://img.shields.io/badge/NEYRONETBOOK-v1.1.0-7c5cff?style=for-the-badge&logo=electron&logoColor=white" alt="Version" />
...
## 🚀 Что нового в версии 1.1.0 (Release Notes v1.1.0)

- 🔢 **Исправлен ввод чисел без залипания нуля**: во всех полях ввода (ставки за смену/час, премии, расходы, суммы переводов, цели и депозиты) на десктопе и в мобильном приложении устранен баг с ведущим нулём `0`. Теперь при наборе любой цифры значение форматируется корректно без `0222`.
- 📅 **Мобильная адаптация календаря смен**:
  - Заменена громоздкая панель 5 карточек на компактную информативную строку статуса месяца: `+36 000 ₽ · 180ч · 15 см. | 15 вых.`
  - Оптимизирована сетка дней: устранены наплывы текста, обрезка цифр и перегрузка экрана на любых смартфонах (от 320px).
  - Скрыта десктопная кисть быстрой раскраски на смартфонах, освободив полезное пространство.
- 💳 **Переосмысленный мобильный финансовый менеджер**:
  - Десктопный сайдбар категорий скрыт на экранах телефонов для мгновенного доступа к списку операций.
  - Премиальный Hero-виджет баланса с разбивкой на доходы, расходы и начисления со смен.
  - Быстрые чипы ввода сумм (`+100`, `+500`, `+1 000`, `+3 000`, `+5 000`).
- 🔄 **Интерактивный генератор циклов и графиков смен**:
  - Быстрое заполнение графиков (2/2, День/Ночь/48, 1/3, 1/2, 5/2, Custom) прямо в мобильном окне с привязкой к выбранной дате и периоду до года.
- 🧠 **Холст (Canvas) — визуальные заметки нового поколения**:
  - Устранена блокировка жестов Android WebView — карточки плавно перетаскиваются с аппаратным ускорением 120 FPS.
  - Интерактивный режим связывания карточек стрелками с выбором семантики связи (`➔ Следствие`, `💡 Пример`, `❓ Вопрос`, `⚡ Противоречие`, `🔗 Связь`).
  - Полноэкранный редактор карточки как развернутой Markdown-заметки с панелью форматирования и экспортом в базу знаний (Блокнот).
  - Готовые образовательные шаблоны: Конспект темы, Флэш-карточка, Анализ решений, Термины.
- 📝 **Сплит-режим редактора заметок**:
  - Интегрирован живой рендерер Markdown-блоков (`LiveDocumentView`) в окне сплит-просмотра с адаптивной компоновкой для мобильных экранов.
